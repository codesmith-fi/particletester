...
namespace Codesmith.SmithNgine.MathUtil
{
    ...
    public static class Interpolations
    {
        ...
        public static Point GetRandomRectanglePoint(Rectangle rect, PseudoRandom r)
        {
            float x = r.NextFloat() * rect.Width;
            float y = r.NextFloat() * rect.Height;
            ...
            ref float transitionValue, float minLimit, float maxLimit )
        {
                ...
                ((direction < 0) && (transitionValue < minLimit)))
            {
                ...
                transitionValue = MathFunctions.Clamp(transitionValue, minLimit, maxLimit);
                return false;
            }
            // Still transitioning
            return true;
            ...
            int direction, ref float transitionValue)
        {
            ...
            {
                // We are done transitioning in or out, clamp the value and end transition
                transitionValue = MathFunctions.Clamp(transitionValue, 0.0f, 1.0f);
                return false;
            }
            // Still transitioning
            return true;
